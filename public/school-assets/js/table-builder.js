...
(() => {
  ...
  function renderPreview(){
    ...
    const thead=mergeEquivalentHeaders(headerRows()).map(row=>`<tr>${row.map(header=>{
      const classes=[header.kind,header.vertical?'vertical-header':'',header.total?'total-column':'',header.studentName?'student-name-column':'',header.studentNumber?'student-number-column':''].filter(Boolean).join(' ');
    ...
    const tbody=names.map((name,row)=>`<tr data-student-id="${row+1}">${columns.map(column=>{
      ...
      const classes=column.type==='student_number'?'student-number-column':column.type==='student_name'?'student-name-column':column.type==='calculated_total'?'total-column':'';
      return `<td${classes?` class="${classes}"`:''} data-column-key="${escapeHtml(column.column_key)}"><span class="cell-value">${escapeHtml(value)}</span></td>`;
    }).join('')}</tr>`).join('');
