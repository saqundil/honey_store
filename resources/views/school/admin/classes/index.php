...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	try {
		...
		school_redirect('admin/classes/index.php');
...
<div class="section-head">
	<div>
		<p class="eyebrow"><?= school_e(($state['terms'][0]['academic_year_name'] ?? '') . ' · ' . ($state['terms'][0]['name'] ?? '')) ?></p>
		<h2>الصفوف</h2>
	</div>
...
<?php if (!$state['terms'] || !$state['stages']): ?>
	<div class="empty-state">
		...
		<a class="button primary" href="<?= school_e(school_url('admin/setup.php')) ?>">فتح الإعداد</a>
	</div>
<?php else: ?>
	<section class="panel add-class-panel" <?= $error ? '' : 'hidden' ?>>
		...
		<form method="post" class="form-grid">
			...
			<label>المرحلة<select name="stage_id" required><?php foreach ($state['stages'] as $stage): ?><option value="<?= (int) $stage['id'] ?>"><?= school_e($stage['name']) ?></option><?php endforeach; ?></select></label>
			<label>الفصل الدراسي<select name="term_id" required><?php foreach ($state['terms'] as $term): ?><option value="<?= (int) $term['id'] ?>"><?= school_e($term['academic_year_name'] . ' · ' . $term['name']) ?></option><?php endforeach; ?></select></label>
	...
	<?php if ($byStage): ?>
		<?php foreach ($byStage as $stageName => $items): ?>
			<section class="stage-block">
				<h3 class="stage-title"><?= school_e($stageName) ?> <span><?= count($items) ?></span></h3>
