...
<?php if ($contextClass): ?>
	<div class="section-head">
		<div>
			<p class="eyebrow"><a href="<?= school_e(school_url('admin/gradebook/class.php?id=' . $contextClassId)) ?>">→ <?= school_e($contextClass['name']) ?></a></p>
			<h2>طلاب <?= school_e($contextClass['name']) ?></h2>
		</div>
		<a class="button" href="<?= school_e(school_url('admin/students/index.php')) ?>">كل الطلاب</a>
	</div>
<?php endif; ?>
<div class="split-view">
    <div>
        <section class="panel">
            ...
            <form method="post" class="form-grid">
                ...
                <label class="full">رقم الطالب<input name="student_number" required value="<?= school_e($editing['student_number'] ?? '') ?>" placeholder="مثال: 1024"></label>
                <label class="full">اسم الطالب<input name="name" required value="<?= school_e($editing['name'] ?? '') ?>" placeholder="الاسم الرباعي"></label>
                ...
                <div class="button-row full">
                    ...
                    <?php if ($editing): ?><a class="button ghost" href="<?= school_e(school_url('admin/students/index.php')) ?>">إلغاء</a><?php endif; ?>
                </div>
            </form>
        </section>

        <section class="panel import-panel">
            <h2><?= $contextClass ? "استيراد الطلاب إلى " . school_e($contextClass["name"]) : "استيراد الطلاب" ?></h2>
    ...
    <section class="panel grow">
        <form class="filters" role="search">
            <input name="q" type="search" value="<?= school_e($_GET['q'] ?? '') ?>" placeholder="ابحث بالاسم أو الرقم" aria-label="بحث عن طالب">
        ...
        <?php else: ?>
            <div class="empty-state">
                ...
                <?php if (($_GET['q'] ?? '') !== ''): ?>
                    <a class="button" href="<?= school_e(school_url('admin/students/index.php' . ($contextClassId ? '?class_id=' . $contextClassId : ''))) ?>">مسح البحث</a>
                <?php endif; ?>
            </div>
        <?php endif; ?>
    </section>
