...
if($_SERVER['REQUEST_METHOD']==='POST'){verify_csrf($_POST['csrf_token']??null);$name=trim((string)($_POST['name']??''));if($name!==''){db()->prepare('INSERT INTO subjects(name) VALUES(?) ON DUPLICATE KEY UPDATE status="active"')->execute([$name]);}school_redirect('admin/subjects/index.php');}
