...
<section class="class-context" aria-labelledby="class-title">
    <div class="class-context-title">
        <h2 id="class-title"><?= school_e($class['name']) ?></h2>
        <p><?= school_e($class['stage_name'] . ' · ' . $class['term_name'] . ' ' . $class['academic_year_name']) ?></p>
    </div>
    <div class="class-context-meta">
        ...
        <a class="tbtn tbtn-primary" href="<?= school_e(school_url('admin/gradebook/new_exam.php?class_id=' . $classId)) ?>">
...
<nav class="class-actions" aria-label="إجراءات الصف">
    <a class="class-action" href="<?= school_e(school_url('admin/students/index.php?class_id=' . $classId)) ?>">
        <strong>الطلاب</strong>
        <small><?= (int) $class['students_count'] ?> مسجّلًا · إضافة واستيراد</small>
    </a>
    <?php if ($counts['all'] > 0): ?>
        <a class="class-action" href="<?= school_e(school_url('admin/gradebook/report.php?id=' . (int) $class['active_assignment_id'])) ?>">
    ...
    <?php if ($exams): ?>
        ...
        <div class="exam-grid" role="list">
            <?php foreach ($exams as $exam):
                $entryUrl = school_url('admin/gradebook/entry.php?id=' . (int) $exam['id']);
            ...
            ?>
                <article class="exam-card" role="listitem" data-status="<?= school_e($status) ?>">
                    <div class="exam-card-primary">
                        <div class="exam-card-title">
                            <a href="<?= school_e($entryUrl) ?>"><?= school_e($exam['assessment_name']) ?></a>
                        </div>
                        <div class="exam-card-meta">
                            <?php if ($exam['template_name']): ?>
                                <span><?= school_e($exam['template_name']) ?></span>
                                <span class="dot" aria-hidden="true"></span>
                            <?php endif; ?>
                            <?php if ($exam['exam_date']): ?>
                                <time datetime="<?= school_e($exam['exam_date']) ?>"><?= school_e($dateDisplay) ?></time>
                            <?php else: ?>
                                <time datetime="<?= school_e($exam['created_at']) ?>" title="تاريخ الإنشاء"><?= school_e($dateDisplay) ?></time>
                            <?php endif; ?>
                        </div>
                    </div>

                    <div class="exam-card-actions">
                        <span class="exam-card-status" data-status="<?= school_e($status) ?>">
                            <?= school_e($statusLabels[$status] ?? $status) ?>
                        </span>
                        <a class="exam-card-open" href="<?= school_e($entryUrl) ?>">
                            <span>فتح</span>
...
<script>
    window.APP = {
        baseUrl: <?= json_encode(school_url()) ?>,
        csrf: <?= json_encode(school_csrf_token()) ?>
    };
