...
<div class="assessment-editor" id="assessment-editor">
    ...
    <section class="panel scheme-details">
        ...
        <div class="form-grid three">
            <label>اسم المخطط<input id="scheme-name" value="<?= e($initial['name']) ?>" required></label>
            <label>الفصل الدراسي<select id="scheme-term" required><?php foreach ($setupState['terms'] as $term): ?><option value="<?= (int) $term['id'] ?>" <?= (int) $initial['academic_term_id'] === (int) $term['id'] ? 'selected' : '' ?>><?= e($term['academic_year_name'] . ' · ' . $term['name']) ?></option><?php endforeach; ?></select></label>
            <label>المادة<select id="scheme-subject" required><?php foreach ($subjects as $subject): ?><option value="<?= (int) $subject['id'] ?>" <?= (int) $initial['subject_id'] === (int) $subject['id'] ? 'selected' : '' ?>><?= e($subject['name']) ?></option><?php endforeach; ?></select></label>
        </div>
        <label>وصف اختياري<textarea id="scheme-description" rows="2"><?= e($initial['description']) ?></textarea></label>
    </section>
