...
<div class="section-head">
    ...
    <div class="quick-actions">
        <a class="button" href="<?= e(url('admin/templates/import.php')) ?>">استيراد من جدول أو PDF</a>
        <a class="button primary" href="<?= e(url('admin/templates/edit.php')) ?>">قالب جديد</a>
    </div>
</div>
            ...
            <?php foreach ($templates as $template): ?>
                <tr>
                    <td>
                        ...
                        <?php if ($template['description']): ?><small><?= e($template['description']) ?></small><?php endif; ?>
                    </td>
                    <td class="num">v<?= (int) $template['version_number'] ?></td>
                    <td><span class="status <?= e($template['status']) ?>"><?= $template['status'] === 'active' ? 'نشط' : 'معطل' ?></span></td>
                    <td class="num"><?= e($template['updated_at']) ?></td>
                    <td class="actions-cell">
                        <a href="<?= e(url('admin/templates/edit.php?id=' . $template['id'])) ?>">تعديل</a>
                        <a href="<?= e(url('admin/templates/preview.php?id=' . $template['id'])) ?>">معاينة</a>
                        <a href="<?= e(url('admin/reports/create.php?template=' . $template['id'])) ?>">استخدام</a>
                        <button class="link-button template-copy" data-id="<?= (int) $template['id'] ?>" data-name="<?= e($template['name']) ?>">نسخ</button>
                        <button class="link-button template-status" data-id="<?= (int) $template['id'] ?>" data-name="<?= e($template['name']) ?>"><?= $template['status'] === 'active' ? 'تعطيل' : 'تفعيل' ?></button>
                        <button class="link-button danger template-delete" data-id="<?= (int) $template['id'] ?>" data-name="<?= e($template['name']) ?>">حذف</button>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
...
<?php else: ?>
    <div class="empty-state">
        ...
        <div class="quick-actions">
            <a class="button" href="<?= e(url('admin/templates/import.php')) ?>">استيراد جدول</a>
            <a class="button primary" href="<?= e(url('admin/templates/edit.php')) ?>">إنشاء قالب</a>
        </div>
    </div>
<?php endif; ?>
