...
<div class="setup-shell">
    ...
    <section class="setup-step">
        ...
        <form method="post" class="form-grid setup-form">
            <?= csrf_field() ?><input type="hidden" name="action" value="academic_context">
            <label>العام الدراسي<input name="academic_year" required pattern="\d{4}/\d{4}" placeholder="2027/2026" value="<?= e($state['years'][0]['name'] ?? '') ?>"></label>
        ...
        <?php if ($state['terms']): ?><div class="setup-items"><?php foreach ($state['terms'] as $term): ?><span><?= e($term['academic_year_name'] . ' · ' . $term['name']) ?></span><?php endforeach; ?></div><?php endif; ?>
    </section>
    ...
    <section class="setup-step <?= !$state['terms'] ? 'is-disabled' : '' ?>">
        ...
        <?php if ($state['stages']): ?><div class="setup-items"><?php foreach ($state['stages'] as $stage): ?><span><?= e($stage['name']) ?></span><?php endforeach; ?></div><?php endif; ?>
    </section>
    ...
    <section class="setup-step <?= !$state['stages'] ? 'is-disabled' : '' ?>">
        ...
        <form method="post" class="form-grid setup-form">
            <?= csrf_field() ?><input type="hidden" name="action" value="class">
            <label>الفصل<select name="term_id" required <?= !$state['stages'] ? 'disabled' : '' ?>><?php foreach ($state['terms'] as $term): ?><option value="<?= (int) $term['id'] ?>"><?= e($term['academic_year_name'] . ' · ' . $term['name']) ?></option><?php endforeach; ?></select></label>
            <label>المرحلة<select name="stage_id" required <?= !$state['stages'] ? 'disabled' : '' ?>><?php foreach ($state['stages'] as $stage): ?><option value="<?= (int) $stage['id'] ?>"><?= e($stage['name']) ?></option><?php endforeach; ?></select></label>
        ...
        <?php if ($state['classes']): ?><div class="setup-items"><?php foreach ($state['classes'] as $class): ?><span><?= e($class['stage_name'] . ' · ' . $class['name']) ?></span><?php endforeach; ?></div><?php endif; ?>
    </section>

    <?php if ($state['complete']): ?>
        <form method="post" class="setup-finish"><?= csrf_field() ?><input type="hidden" name="action" value=""><button class="button primary" name="finish" value="1">الانتقال إلى لوحة التحكم</button></form>
    <?php endif; ?>
</div>
<?php page_footer(); ?>
