...
final class TableRenderer
{
    ...
    public function render(array $template, array $students = [], array $studentValues = [], bool $editable = false, int $minimumRows = 0): string
    {
        ...
        foreach ($layout['columns'] as $column) {
            ...
            echo '<col style="width:' . e($this->formatMark(($width / $totalWidth) * 100)) . '%">';
        }
        echo '</colgroup><thead>';
        foreach ($layout['rows'] as $row) {
            ...
            foreach ($row as $cell) {
                ...
                echo '<th class="' . e(trim($cell['kind'] . $vertical . $total)) . '" colspan="' . (int) $cell['colspan'] . '" rowspan="' . (int) $cell['rowspan'] . '">';
                echo '<span><span class="header-label">' . e($cell['label']) . '</span>';
                if ($column && $column['max_mark'] !== null && $column['max_mark'] !== '') echo '<small class="header-mark" dir="ltr">(' . e($this->formatMark($column['max_mark'])) . ')</small>';
                echo '</span></th>';
            }
            echo '</tr>';
        }
        ...
        foreach ($students as $index => $student) {
            ...
            foreach ($layout['columns'] as $column) {
                ...
                echo '<td' . $total . ' data-column-key="' . e($key) . '">';
                if ($editable && $column['type'] === 'manual_mark') {
                    echo '<input class="mark-input" type="number" min="0" max="' . e($column['max_mark']) . '" step="' . e($column['step_value'] ?? '0.25') . '" value="' . e($value) . '">';
                } elseif ($editable && in_array($column['type'], ['text', 'date'], true)) {
                    echo '<input class="value-input" type="' . ($column['type'] === 'date' ? 'date' : 'text') . '" value="' . e($value) . '">';
                } else {
                    echo '<span class="cell-value">' . e($value) . '</span>';
                }
                echo '</td>';
            }
            echo '</tr>';
        ...
        for ($index = count($students); $index < $minimumRows; $index++) {
            ...
            foreach ($layout['columns'] as $column) {
                ...
                $total = $column['type'] === 'calculated_total' ? ' class="total-column"' : '';
                echo '<td' . $total . ' data-column-key="' . e($column['column_key']) . '"><span class="cell-value">' . e($value) . '</span></td>';
            }
            echo '</tr>';
        }
        echo '</tbody></table>';
